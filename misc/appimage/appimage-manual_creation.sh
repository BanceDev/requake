...
if [ $FAIL -eq 0 ];then
	...
	"${APPDIR}/usr/bin/requake-linux-'$ARCH'" $*
else
	echo "executing with appimage libc"
	export LD_LIBRARY_PATH="${LD_LIBRARY_PATH}:${APPDIR}/usr/lib-override"
	"${APPDIR}/usr/lib-override/ld-linux-'$ARCHDASH'.so.2" "${APPDIR}/usr/bin/requake-linux-'$ARCH'" $*
fi
exitstatus=$?
...
if ! [[ -x "$EXECUTABLE" ]]; then
	...
	EXECUTABLE="build/requake-linux-$ARCH"
fi
...
echo "$DESKTOP_ENTRY" > "$DIR/AppDir/io.github.reQuake.desktop" || exit 4
...
sed 's,EZQUAKE_VERSION,'$VERSION-$REVISION',g;s,EZQUAKE_DATE,'$(date +%F)',g' "$DIR/misc/appimage/ezquake.appdata.xml.template" > "$DIR/AppDir/usr/share/metainfo/requake.appdata.xml"
ldd "$DIR/AppDir/usr/bin/requake-linux-$ARCH" | \
...
cd "$DIR" || exit 5
./appimagetool AppDir reQuake-$ARCH.AppImage || exit 7
