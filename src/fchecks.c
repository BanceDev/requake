...
{
	Cbuf_AddText (va("say {reQuake &c9f6%s&r &c777%s&r " QW_PLATFORM ":" QW_RENDERER "}\n", VERSION_NUMBER, VersionHash()));
}
...
static void FChecks_FServerResponse (void)
{
	...
	Cbuf_AddText(va("say reQuake f_server response: %s\n", text));
}
...
static qbool FChecks_CheckFRulesetRequest (const char *s)
{
	...
	shorthash[5] = '\0';
	snprintf(versionwithhash, sizeof(versionwithhash), "re&c9f6%s-%s&r", VERSION_NUMBER, shorthash);
