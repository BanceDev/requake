...
#define    QUICKPLAY_ITEMS    4
int    m_quickplay_cursor;
menu_window_t m_quickplay_window;
static qbool m_quickplay_big = false;
static bigmenu_items_t m_solo_items[] = {
    {"Duel", NULL},
    {"Aim Arena", NULL}, 
};

static bigmenu_items_t m_team_items[] = {
    {"2on2", NULL},
    {"4on4", NULL}, 
};
static qbool failed_to_find = false;
static double failed_to_find_time = 0;

void M_Menu_QuickPlay_f (void) {
    M_EnterMenu(m_quickplay);
}

void M_QuickPlay_Draw (void) {
    int f = (int)(curtime * 10)%6;
    mpic_t *solo;
    mpic_t *team;
    int solo_width, solo_height;
    int team_width, team_height;
    int solo_x, solo_y;
    int team_x, team_y;
    int column_spacing = 160; // Distance between columns
    int cursor_x, cursor_y;
    int items_per_column = BIGMENU_ITEMS_COUNT(m_solo_items);

    solo = Draw_CachePic (CACHEPIC_QP_SOLO);
    solo_x = 16;
    solo_y = 4 + m_yofs;

    team = Draw_CachePic (CACHEPIC_QP_TEAM);
    team_x = solo_x + column_spacing;
    team_y = solo_y;

    // Draw solo items below solo header
    m_quickplay_big = true;
    m_quickplay_window.x = solo_x + 8;
    m_quickplay_window.y = 144 + m_yofs;
    M_BigMenu_DrawItems(m_solo_items, BIGMENU_ITEMS_COUNT(m_solo_items),
        m_quickplay_window.x, m_quickplay_window.y, 
        &solo_width, &solo_height);

    // Draw team items below team header
    M_BigMenu_DrawItems(m_team_items, BIGMENU_ITEMS_COUNT(m_team_items),
        team_x + 8, 144 + m_yofs, 
        &team_width, &team_height);

    Draw_Pic(solo_x, solo_y, solo);
    Draw_Pic(team_x, team_y, team);
    // Update window dimensions for mouse selection (using solo column for now)
    m_quickplay_window.w = solo_width;
    m_quickplay_window.h = solo_height;

    // Determine cursor position based on which column
    int itemheight = solo_height / items_per_column;

    if (m_quickplay_cursor < items_per_column) {
        // Cursor is in solo column
        cursor_x = solo_x - 12;
        cursor_y = 144 + m_quickplay_cursor * itemheight;
    } else {
        // Cursor is in team column
        cursor_x = team_x - 12;
        cursor_y = 144 + (m_quickplay_cursor - items_per_column) * itemheight;
    }
    Draw_TransPic (cursor_x, cursor_y + m_yofs, Draw_CachePic(CACHEPIC_MENUDOT1 + f));

    if (failed_to_find && (curtime - failed_to_find_time > 2.0)) {
        failed_to_find = false;
    }
    if (failed_to_find) {
	    M_DrawTextBox (60, 10*8, 23, 4);
	    M_PrintWhite (88, 12*8, "Failed to find server");
	    M_PrintWhite (88, 13*8, "for quickplay mode.");
    }

}


void M_QuickPlay_Key (int key) {
    ...
    switch (key) {
        ...
        case K_MWHEELDOWN:
            S_LocalSound ("misc/menu1.wav");
            if (++m_quickplay_cursor >= QUICKPLAY_ITEMS)
                m_quickplay_cursor = 0;
            break;

        case K_UPARROW:
        case K_MWHEELUP:
            S_LocalSound ("misc/menu1.wav");
            if (--m_quickplay_cursor < 0)
                m_quickplay_cursor = QUICKPLAY_ITEMS - 1;
            break;
        ...
        case K_ENTER:
	case K_MOUSE1:
	    switch (m_quickplay_cursor) {
		case 0:
		    failed_to_find = QP_QuickPlay("1on1");
		    if (failed_to_find) failed_to_find_time = curtime;  // Add this
		    break;
		case 1:
		    failed_to_find = QP_QuickPlay("dmm4");
		    if (failed_to_find) failed_to_find_time = curtime;  // Add this
		    break;
		case 2:
		    failed_to_find = QP_QuickPlay("2on2");
		    if (failed_to_find) failed_to_find_time = curtime;  // Add this
		    break;
		case 3:
		    failed_to_find = QP_QuickPlay("4on4");
		    if (failed_to_find) failed_to_find_time = curtime;  // Add this
		    break;
	    }
    }
...
void M_Init (void) {
    ...
    Cmd_AddCommand ("menu_qplay", M_Menu_QuickPlay_f);
